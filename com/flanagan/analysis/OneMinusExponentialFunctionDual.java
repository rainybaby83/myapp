...
class OneMinusExponentialFunctionDual implements RegressionFunction3 {
    private boolean scaleOption = true;
    private double scaleFactor = 1.0D;
    private double[][] xErrors = (double[][])null;
    private double[] yErrors = null;

    OneMinusExponentialFunctionDual() {
    }
    ...
    public double[] function(double[] var1, double[] var2, int var3) {
        ...
        double[] var6 = new double[]{var4 * (1.0D - Math.exp(var1[1] * var2[0])), 0.0D};
        if (var3 >= 0) {
            var6[1] = -var4 * var1[1] * Math.exp(var1[1] * var2[0]);
